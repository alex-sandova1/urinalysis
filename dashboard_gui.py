"""Urinalysis Diagnostic Analytics desktop dashboard GUI."""
...
class Dashboard(tk.Tk):
    ...
    def build_sidebar(self, parent: tk.Widget) -> None:
        side = tk.Frame(parent, bg=NAVY, width=295)
    ...
    def build_filters(self, parent: tk.Widget) -> None:
        ...
        entry = tk.Entry(search, textvariable=self.search_var, width=25, bd=1, relief="solid", font=("Arial", 10), fg=TEXT,bg="white",insertbackground=TEXT)
        entry.pack(ipady=7)
    ...
    def draw_cards(self) -> None:
        ...
        normal = total - abnormal
        items = [
            ("⚗", "Total Samples", str(total), "Filtered dataset", BLUE),
            ("✓", "Normal Samples", str(normal), f"{normal / total:.1%}" if total else "—", GREEN),
            ("⚠", "Abnormal Samples", str(abnormal), f"{abnormal / total:.1%}" if total else "—", RED),
            ("⌁", "Parameters Analyzed", "10", "Quality measures", AMBER),
    ...
    def chart_canvas(self, panel: tk.Frame) -> tk.Canvas:
        for widget in panel.winfo_children()[1:]:
            widget.destroy()
    ...
    def draw_parameter_chart(self) -> None:
        for widget in self.parameter_panel.winfo_children()[1:]:
            widget.destroy()
    ...
    def draw_averages(self) -> None:
        for widget in self.averages.winfo_children():
            widget.destroy()
        frame = self.filtered
        data = [
            ("Average pH", f"{frame['pH'].mean():.2f}" if len(frame) else "—"),
            ("Avg Specific Gravity", f"{frame['Specific Gravity'].mean():.3f}" if len(frame) else "—"),
            ("Average WBC (HPF)", f"{frame['WBC'].map(range_upper).mean():.1f}" if len(frame) else "—"),
            ("Average RBC (HPF)", f"{frame['RBC'].map(range_upper).mean():.1f}" if len(frame) else "—"),
            ("Protein Positive Rate", f"{(frame['Protein'].map(lambda v: clean(v) != 'NEGATIVE')).mean():.1%}" if len(frame) else "—"),
            ("Glucose Positive Rate", f"{(frame['Glucose'].map(lambda v: clean(v) != 'NEGATIVE')).mean():.1%}" if len(frame) else "—"),
        ]
        for label, value in data:
            item = tk.Frame(self.averages, bg="#f4f8ff")
            item.pack(side="left", fill="x", expand=True)
            tk.Label(item, text=label, bg="#f4f8ff", fg=TEXT, font=("Arial", 8, "bold")).pack(anchor="w", padx=6, pady=(6, 0))
            tk.Label(item, text=value, bg="#f4f8ff", fg=TEXT, font=("Arial", 14, "bold")).pack(anchor="w", padx=6, pady=(0, 6))
